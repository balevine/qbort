// Composes the user's TICKET_PROMPT.md (the creative / distribution half) with the engine-enforced
// output requirements (schema, roster, per-batch response targets, allowed statuses). The user's
// text is never overridden. Requirements are appended and clearly delimited.
...
/**
 * @typedef {object} StaffPromptInput
...
/**
 * @typedef {object} CompilePromptInput
 * @property {string} editablePrompt the user's editable creative/distribution prompt
 * @property {number} batchCount how many tickets this batch should produce
 * @property {string[]} [scenarios] one-line scenarios dealt to this batch (scenario i → ticket i)
 * @property {StaffPromptInput} staff
 */

/** The exact JSON shape we ask the model to return (no `id`, since the engine assigns ids). */
...
/**
 * The static half of the staff section (identical across every batch): whether staff replies are
 * enabled, the roster, and (when no per-ticket targets are supplied) the average directive.
 * Per-ticket targets vary per batch and live in the dynamic suffix instead.
 * @param {StaffPromptInput} staff
 * @returns {string}
 */
...
/**
 * Static prefix: the user's editable text + requirements that never change per batch.
 * @param {CompilePromptInput} input
 * @returns {string}
 */
function staticPrefix(input) {
  ...
  const requirements = [
    DELIMITER,
    'OUTPUT REQUIREMENTS (enforced by the engine — follow exactly)',
    DELIMITER,
    ...
    'C. Do NOT include an "id" field — the engine assigns ids.',
...
/**
 * Dynamic suffix: the small per-batch instructions (count, per-ticket response targets, scenarios).
 * @param {CompilePromptInput} input
 * @returns {string}
 */
...
/**
 * Compose the prompt as a static prefix (identical across every batch of a run) plus a small
 * per-batch dynamic suffix, joined with a blank line. The engine compiles the prefix once in `plan`
 * and reuses it for every batch, which is why this returns the halves rather than one string.
 * @param {CompilePromptInput} input
