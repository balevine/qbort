...
async function cmdAssemble(args) {
  ...
  for (const b of manifest.batches) {
    ...
    const raw = await readText(b.batchFile)
  ...
  // Check our own output against the format's definition before writing it. Every other guard here
  // validates the *model*; this one validates the engine, and it is the only thing standing between
  // a shape change made here and a viewer in another repo that silently stops loading the file.
